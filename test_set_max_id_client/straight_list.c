...
int straight_list_create(list_t* lp, size_t size, straight_list_copy_t copy, straight_list_destroy_t destroy)
{
	lp->current = NULL;
	lp->first = NULL;
	lp->size = size;
...
void straight_list_destroy(list_t *lp)
{
	straight_list_clear(lp);
}

void straight_list_clear(list_t *lp)
{
...
int straight_list_is_empty(const list_t *lp)
{
	return (lp->first == NULL);
}

void straight_list_get(const list_t *lp, void *data)
{
	lp->copy(data, lp->current->data);
}

int straight_list_move(list_t *lp, straight_list_movement_t m)
{
	switch (m)
	{
...
void straight_list_delete(list_t *lp)
{
...
int straight_list_insert(list_t *lp, straight_list_movement_t m, const void* data)
{
...
int straight_list_update(list_t* l, const void* data) {
	if(!l->first) {
