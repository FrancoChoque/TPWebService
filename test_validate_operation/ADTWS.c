...
int get_all_operations(ADTWS* ws){
	...
	if((str = (char*) malloc(ws->operation_list.data_size)) == NULL){ /*abstraction rape*/
		return ERROR_MEMORY_SHORTAGE;
	}
...
int log_operation(ADTWS ws){
	...
	fprintf(fp,"%s;%s;\n%s;\n%s",ADTWS_Op_get_operation_time(ws.operation_t),ADTWS_Op_get_operation(ws.operation_t),ADTWS_Op_get_request(ws.operation_t),ADTWS_Op_get_response(ws.operation_t));
